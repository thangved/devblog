...
export async function getServerSideProps(context) {
	try {
		...
		const info = await (
			await axios.get(
				`${constants.api}/url/info?url=${res.data.data.url}`
			)
		).data.data;
	...
	return (
		<Container>
			...
			<Box w="100%" border="1px solid #ddd" mt="2">
				<Image w="100%" src={data.info.img} alt={data.title} />

				<Box p="2" background="gray.100" borderTop="1px solid #ddd">
					<Heading size="sm">
						{data.title || data.description}
					</Heading>
					<Text>{data.description || data.title}</Text>
				</Box>
			</Box>
			...
			<Countdown
				...
				renderer={({ seconds, completed }) => {
					return completed ? (
						...
						<Stat mt="2">
							<StatLabel>Chuyển hướng sau</StatLabel>
							<StatNumber>{seconds} giây</StatNumber>
						</Stat>
					);
				}}
			/>
