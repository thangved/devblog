...
const CreateLink = ({ setMyLinks }) => {
	...
	const onSubmit = async () => {
		...
		try {
			...
			if (!res.data.success) return toast.error(data.message);
	...
	const getPreview = async (url) => {
		try {
			const data = (
				await axios.get(`${constants.api}/url/info?url=${url}`)
			).data;
	...
	return (
		<Container p="0">
			<VStack pt="2" pb="2" m="0">
				...
				{preview.show ? (
					<Box width="100%" border="1px solid #ddd">
						{preview.img && (
							<Image
								width="100%"
								src={preview.img}
						...
						<Box p="2" backgroundColor="gray.100">
							...
							<Text>{preview.description || preview.title}</Text>
						</Box>
					</Box>
				) : (
					<Stack width="100%">
				...
				<Button colorScheme="blue" isFullWidth onClick={onSubmit}>
					Tạo
				</Button>
			</VStack>
		</Container>
