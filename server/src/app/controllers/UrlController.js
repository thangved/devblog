...
class UrlController {
	...
	async myLinks(req, res) {
		...
		try {
			...
			const data = await (
				await ModelUrl.find({ author: existingUser._id }).populate(
					"author"
				)
			).map((link) => ({
				...link.toObject(),
	...
	async convert(req, res) {
		...
		try {
			const existingUrl = await ModelUrl.findOne({ slug }).populate(
				"author"
			);
	...
	async remove(req, res) {
		...
		try {
			...
			ModelUrl.findOneAndDelete(
				{ _id, author: existingUser._id },
				(error) => {
					if (error)
						return res.send({
							success: false,
